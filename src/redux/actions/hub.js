...
export const loginAndCreateBucket = () => async (dispatch) => {
  dispatch({
    type: types.TITLE_CREATE_BUCKET,
    payload: "Creating Hub Identity...",
  });
...
export const registerMeme = (payload) => async (dispatch) => {
  ...
  dispatch({
    type: types.TITLE_CREATE_BUCKET,
    payload: "Adding meme to Textile Bucket...",
  });
  const result = await hubClient.addFileToBucket(name, fileBuffer);

  dispatch({
    type: types.TITLE_CREATE_BUCKET,
    payload: "Registering Media NFT on Blockchain...",
  });
